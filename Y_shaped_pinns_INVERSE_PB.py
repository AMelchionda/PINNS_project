...
class OneDBioPINN:
    ...
    def __init__(self, X_measurement_vessel1, # length vessel 1 ecc..
        ...
        config = tf.ConfigProto(
            log_device_placement=True)  # To find out which devices your operations and tensors are assigned to
        self.sess = tf.Session(config=config)  # create the environment for tensorflow

        # Vessels cross-sectional areas - sect. (3.1)
        # self.A_01 = tf.Variable([0.01], dtype=tf.float32)
        # self.A_02 = tf.Variable([0.001], dtype=tf.float32)
        # self.A_03 = tf.Variable([0.01], dtype=tf.float32)
        ...
        self.beta1V = tf.Variable([8.0], dtype=tf.float32)
        self.beta2V = tf.Variable([8.0], dtype=tf.float32)
        self.beta3V = tf.Variable([8.0], dtype=tf.float32)
        ...
        # characteristic length - see sect. (2.7), eq. (13)
        self.L = np.sqrt(0.333*(self.A_01 + self.A_02 + self.A_03))
        # characteristic time - see sect. (2.7)
        ...
        # Evaluate NN sol for colloc. pts
    ...
    def pinn_vessel1(self, x, t):  # RESIDUI EQ(15)
        ...
        beta1 = 10 ** self.beta1V
        r_p = beta1*(tf.sqrt(A*self.A0) - tf.sqrt(self.A_01))  # residuo terza eq dell'eq(15) #p?
    ...
    def pinn_vessel2(self, x, t):
        ...
        beta2 = 10 ** self.beta2V
        r_p = beta2*(tf.sqrt(A*self.A0) - tf.sqrt(self.A_02))
    ...
    def pinn_vessel3(self, x, t):
        ...
        beta3 = 10 ** self.beta3V
        r_p = beta3*(tf.sqrt(A*self.A0) - tf.sqrt(self.A_03))
    ...
    def train(self, nIter=20000, learning_rate=1e-3):
        ...
        for it in range(nIter):
            ...
            if it % 1 == 0:
                ...
                loss_value, loss_A, loss_u, loss_r, loss_c, beta_1V, beta_2V, beta_3V = self.sess.run([self.loss,
                                                                                     self.loss_A, 
                                                                                     self.loss_u,
                                                                                     self.loss_residual,
                                                                                     self.loss_interface,
                                                                            self.beta1V, self.beta2V, self.beta3V], tf_dict)
    
                print('It: %d, Loss: %.3e, Loss_A: %.3e, Loss_u: %.3e, Loss_r: %.3e, Loss_int: %.3e, beta_1: %.3e, beta_2: %.3e, beta_3: %.3e\
                 Time: %.2f' %
                      (it, loss_value, loss_A, loss_u, loss_r, loss_c, beta_1V, beta_2V, beta_3V, elapsed))
                start_time = timeit.default_timer()
    ...
    #     self.Tmean = param['Parameters'][6]
    #     self.Tstd = param['Parameters'][7]
    #
